    ...
    for (const repo of config.repos) {
      ...
      for (const variable of repo.variables) {
        ...
        try {
          ...
          try {
            const { data: existingVar } = await octokit.rest.actions.getRepoVariable({
              owner: owner,
              repo: repoName,
              name: name,
            });
            ...
            if (err.status === 404) {
              ...
              console.error(`Failed to fetch/update variable ${name}:`, err.message);
            }
          }
        } catch (err) {
          console.error(`Failed to create/update variable ${name}:`, err.message);
        }
      }
    }
  } catch (error) {
